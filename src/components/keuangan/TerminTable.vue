<template>
  ...
  >
    ...
    <template v-slot:top>
      ...
      >
        <v-toolbar-title class="text-uppercase primary--text">
            ...
            <span class="display-1"> TERMIN KEUANGAN</span>
        </v-toolbar-title>        
    ...
    <template v-slot:[`item.jumlahCair`]="{ item }">  
      <div class="text-right" >{{ formattedCurrencyValue(item.jumlahCair) }}</div>        
    </template>
    ...
    <template v-slot:[`item.actions`]="{ item }">        
      ...
      <router-link style="text-decoration:none;" :to="`/KeuanganTermin/${item.id}/${linkNeated(item.description)}`">
          <v-icon                                  
              class="mr-2"
              small
          >
