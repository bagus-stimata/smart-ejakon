  ...
  <div align="center">
    ...
    <v-row no-gutters justify="center" align="center">
      <v-col cols="8">
        <v-file-input
          show-size
          label="Pilih document (pdf, zip, rar, doc, docx)"
          accept=".pdf,.zip,.rar,.doc,.docx,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document"          
          @change="selectImage"
        ></v-file-input>
      </v-col>
  ...
  methods: {
    ...
    upload() {
      ...
      UploadService.uploadDocument(this.currentImage, (event) => {
        ...
        .catch((err) => {
          this.progress = 0;
          this.message = "Could not upload the document! " + err;
